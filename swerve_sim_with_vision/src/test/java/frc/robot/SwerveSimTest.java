...
public class SwerveSimTest {
    ...
    void translationTest() {
        ...
        try {
            ...
            assertAll(
                    () -> assertEquals(0.033, finalPose.getX(), 0.1, "final x"),
                    () -> assertEquals(0, finalPose.getY(), 0.1, "final y"),
                    () -> assertEquals(0, finalPose.getRotation().getRadians(), 0.1, "final rot"));
        } finally {
            m_swerve.close(); // release the HAL stuff
        }
    }
    ...
    void rotationTest() {
        ...
        try {
            ...
            assertAll(
                    () -> assertEquals(-0.033, m_swerve.m_frontLeft.getPosition().distanceMeters, 0.1,
                            "FL m"),
                    () -> assertEquals(0.033, m_swerve.m_frontRight.getPosition().distanceMeters, 0.1,
                            "FR m"),
                    () -> assertEquals(-0.033, m_swerve.m_backLeft.getPosition().distanceMeters, 0.1, "BL m"),
                    () -> assertEquals(0.033, m_swerve.m_backRight.getPosition().distanceMeters, 0.1,
                            "BR m"));

            // no x or y movement
            assertEquals(0, m_swerve.speeds.vxMetersPerSecond, 1.0,  "chassis speed x");
            assertEquals(0, m_swerve.speeds.vyMetersPerSecond, 1.0, "chassis speed y");
           ...
           //assertEquals(3.087, m_swerve.speeds.omegaRadiansPerSecond, 0.1, "chassis speed omega");
            // look at the pose we're maintaining
            assertAll(
                    () -> assertEquals(0, m_swerve.getPose().getX(), 0.1, "pose x"),
                    () -> assertEquals(0, m_swerve.getPose().getY(), 0.1, "pose y"),
                    () -> assertEquals(0.062, m_swerve.getPose().getRotation().getRadians(), 0.1, "pose rot"));

            // look at the gyro, note NWU/NED difference, also one is rad the other deg
            //assertAll(
             //       () -> assertEquals(0.062, m_swerve.m_gyro.getRotation2d().getRadians(), 0.1, "gyro rotation"),
              //      () -> assertEquals(-3.537, m_swerve.m_gyro.getAngle(), 0.1, "gyro angle"));

            final Pose2d finalPose = m_swerve.getPose();
            assertAll(
                    () -> assertEquals(0, finalPose.getX(), 0.1, "estimate x"),
                    () -> assertEquals(0, finalPose.getY(), 0.1, "estimate y"),
                    () -> assertEquals(0.062, finalPose.getRotation().getRadians(), 0.1, "estimate rot"));
        } finally {
            m_swerve.close(); // release the HAL stuff
        }
    }
    ...
    void rotatedTranslationTest() {
        ...
        try {
            ...
            assertAll(
                    () -> assertEquals(0, m_swerve.m_frontLeft.getPosition().angle.getRadians(), 0.1,
                            "FL angle"),
                    () -> assertEquals(0, m_swerve.m_frontRight.getPosition().angle.getRadians(), 0.1,
                            "FR angle"),
                    () -> assertEquals(0, m_swerve.m_backLeft.getPosition().angle.getRadians(), 0.1, "BL angle"),
                    () -> assertEquals(0, m_swerve.m_backRight.getPosition().angle.getRadians(), 0.1,
                            "BR angle"));
            // has not moved yet
            assertAll(
                    () -> assertEquals(0, m_swerve.m_frontLeft.getPosition().distanceMeters, 0.001,
            ...
            assertAll(
                    () -> assertEquals(0, odoPose.getX(), 0.1, "x odo pose"),
                    () -> assertEquals(0.033, odoPose.getY(), 0.1, "y odo pose"),
                    () -> assertEquals(Math.PI / 2, odoPose.getRotation().getRadians(), 0.1, "theta odo pose"));
            ...
            // relative.
           // assertEquals(1.667, m_swerve.speeds.vxMetersPerSecond, 0.1, "chassis speed x");
           // assertEquals(0, m_swerve.speeds.vyMetersPerSecond, 0.1, "chassis speed y");
           // assertEquals(0, m_swerve.speeds.omegaRadiansPerSecond, 0.1, "chassis speed omega");

            // pose shows movement in y, also remember rotation PI/2
            assertAll(
                    () -> assertEquals(0, m_swerve.getPose().getX(), 0.1, "pose x"),
                    () -> assertEquals(0.033, m_swerve.getPose().getY(), 0.1, "pose y"), // <= broken?
                    () -> assertEquals(Math.PI / 2, m_swerve.getPose().getRotation().getRadians(), 0.1, "pose rot"));

            // same rotation
            assertAll(
                    () -> assertEquals(Math.PI / 2, m_swerve.m_gyro.getRotation2d().getRadians(), 5,
                            "gyro rotation"),
                    () -> assertEquals(-90, m_swerve.m_gyro.getAngle(), 5, "gyro angle"));

            final Pose2d finalPose = m_swerve.getPose();

            assertAll(
                    () -> assertEquals(0, finalPose.getX(), 0.1, "estimate x"),
                    () -> assertEquals(0.033, finalPose.getY(), 0.1, "estimate y"), // <= broken?
                    () -> assertEquals(Math.PI / 2, finalPose.getRotation().getRadians(), 0.1, "estimate rot"));
        } finally {
            m_swerve.close(); // release the HAL stuff
        }
    }
