...
public class Robot extends TimedRobot {
  ...
  private final DigitalOutput m_output_return;

  public Robot() {
    m_output = new DigitalOutput(0);
    m_output_return = new DigitalOutput(1);
    // m_output.disablePWM();
    // m_output.setPWMRate(1.0);
    ...
    inst.addListener(
        ...
        (event) -> accept(event));
    inst.addListener(
        vision_table.getEntry("led"),
        EnumSet.of(NetworkTableEvent.Kind.kValueAll),
        (event) -> led_accept(event));
  }

  private void led_accept(NetworkTableEvent event) {
    m_output_return.set(event.valueData.value.getBoolean());
  }
  ...
  private void accept(NetworkTableEvent event) {
    try {
      ...
      // m_output_return.set(blips.tags.size() > 0);
  ...
  public void robotPeriodic() {
    ...
    timestamp_publisher.set(fpgaTimestamp);
    m_output.set((int) (10 * fpgaTimestamp) % 10 == 0);
  }
}
